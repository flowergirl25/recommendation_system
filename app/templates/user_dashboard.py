...
<style>
    ...
    .movie-card {
        background: linear-gradient(145deg, #181818 0%, #2F2F2F 100%);
        border-radius: 12px;
        padding: 20px;
        margin: 15px 0;
...
def star_rating_component(label, current_value=0, max_stars=5, movie_id=None, user_email=None):
    """Display clickable emoji stars with immediate save."""
    st.markdown(f"<div class='star-rating'>", unsafe_allow_html=True)
    st.markdown(f"**{label}**")
...
def movie_card(movie, user_email, section_prefix=""):
    """Enhanced Netflix-style movie card component."""
    with st.container():
...
def insights_section(user_email):
    ...
    if res["success"] and res["data"]:
        ...
        with col2:
            st.metric("Average Rating", f"{sum(ratings)/len(ratings):.2f}/5")
        with col3:
            st.metric("Highest Rating", f"{max(ratings)}/5")
        with col4:
            st.metric("Lowest Rating", f"{min(ratings)}/5")
